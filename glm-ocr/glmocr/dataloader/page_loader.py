...
1. Loading various input formats (images, PDFs, doc, docx, pptx, base64, URLs)
2. Converting inputs into a list of PIL Images
3. Building OCR API request payloads

Supported inputs:
- Local file paths (images, PDFs, .doc, .docx, .pptx, .xls, .xlsx, .ods via LibreOffice)
- file:// URLs
...
class PageLoader:
    ...
    def _iter_source(self, source: str):
        ...
        elif os.path.isfile(file_path) and is_document_path(file_path):
            pdf_path, temp_dir = convert_to_pdf(file_path)
            try:
                yield from self._iter_pdf(pdf_path)
            finally:
                try:
    ...
    def _load_source(self, source: str) -> List[Image.Image]:
        ...
        PDFs return all pages; doc/docx/pptx are converted to PDF then loaded;
        images return a single-page list.
        """
        if source.startswith("file://"):
            file_path = source[7:]
        else:
        ...
        # Document formats: convert to PDF then load
        if os.path.isfile(file_path) and is_document_path(file_path):
            pdf_path, temp_dir = convert_to_pdf(file_path)
            try:
