...
class DoclingConfig(_BaseConfig):
    """Configuration for the Docling-based text parser.

    Used by DocumentPipeline when parsing PDF / DOCX documents.
    """

    # Run Docling's built-in OCR (for scanned PDFs without a text layer).
    # Keep False for native-text PDFs and Word documents.
    use_ocr: bool = False

    # Generate full page images inside Docling.
    # Used as a fallback for cropping table regions.
    generate_page_images: bool = True

    # Extract embedded figure images from Docling.
    # Avoids a second page render step for figure blocks.
...
class PipelineConfig(_BaseConfig):
    ...
    # When True, GlmOcr.parse() routes PDF/DOCX through DocumentPipeline
    # (Docling for text  +  GLM-OCR for tables/figures) instead of the
    # classic all-image pipeline.
    enable_document_pipeline: bool = False

    # Docling parser settings (used when enable_document_pipeline=True)
    docling: DoclingConfig = Field(default_factory=DoclingConfig)

    # When True, tables that already have Docling-extracted markdown are NOT
    # re-sent to GLM-OCR (faster but lower table quality for PDFs).
    # When False (default), GLM-OCR always handles tables for max accuracy.
    docling_skip_table_ocr: bool = False
