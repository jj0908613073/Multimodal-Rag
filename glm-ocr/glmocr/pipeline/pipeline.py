...
# Table crops taller than this (pixels) are split into horizontal strips for OCR
_TABLE_CHUNK_HEIGHT = 600
...
class Pipeline:
                ...
                while True:
                    ...
                    if item_type == "region":
                        ...
                        if task_type == "skip":
                            ...
                            task_type == "table"
                            and cropped_image.size[1] > _TABLE_CHUNK_HEIGHT
                        ):
                            future = executor.submit(
                                self._ocr_table_with_chunking, cropped_image, task_type
                            )
    ...
    @staticmethod
    def _split_image_rows(image, max_height: int):
        """Split a PIL image into horizontal strips of at most max_height pixels."""
        w, h = image.size
        return [
            image.crop((0, y, w, min(y + max_height, h)))
            for y in range(0, h, max_height)
        ]

    @staticmethod
    def _merge_table_markdown_chunks(chunks: List[str]) -> str:
        """Merge Markdown table outputs from horizontal row strips.

        Strip 1 is kept in full.  For strips 2+, the header and separator
        rows (lines with '|---|') are dropped so only data rows are appended.
        """
        if not chunks:
            return ""
        result_lines = chunks[0].splitlines()
        for chunk in chunks[1:]:
            lines = chunk.splitlines()
            # Find the first separator row and take everything after it
            data_start = len(lines)
            for i, line in enumerate(lines):
                if line.strip().startswith("|") and "---" in line:
                    data_start = i + 1
                    break
            result_lines.extend(lines[data_start:])
        return "\n".join(result_lines)

    def _ocr_table_with_chunking(self, cropped_image, task_type: str):
        """OCR a table image, splitting into horizontal strips when too tall.

        Returns (response_dict, status_code) with the same shape as
        ocr_client.process() so the result can be consumed by the caller
        without any special-casing.
        """
        _, h = cropped_image.size
        if h <= _TABLE_CHUNK_HEIGHT:
            req = self.page_loader.build_request_from_image(cropped_image, task_type)
            return self.ocr_client.process(req)

        strips = self._split_image_rows(cropped_image, _TABLE_CHUNK_HEIGHT)
        logger.debug("Table chunk: %dpx tall → %d strips", h, len(strips))

        chunk_contents: List[str] = []
        for strip in strips:
            req = self.page_loader.build_request_from_image(strip, task_type)
            response, status_code = self.ocr_client.process(req)
            if status_code == 200:
        ...
        merged = self._merge_table_markdown_chunks(chunk_contents)
        return {"choices": [{"message": {"content": merged}}]}, 200
