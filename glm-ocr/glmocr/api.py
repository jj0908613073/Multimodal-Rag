...
from glmocr.utils.document_converter import is_document_path, convert_to_pdf
...
class GlmOcr:
    ...
    def __init__(
        ...
        *,
        ...
        enable_document_pipeline: Optional[bool] = None,
        ...
        Args:
            ...
            enable_layout: Whether to run PP-Structure layout detection (image path).
            enable_document_pipeline: When True, PDF/DOCX files are parsed through
                      the two-layer DocumentPipeline (Docling text + GLM-OCR visuals)
                      instead of converting everything to images first.
        ...
        # Apply keyword override for enable_document_pipeline
        if enable_document_pipeline is not None:
            self.config_model.pipeline.enable_document_pipeline = enable_document_pipeline
        ...
        self._temp_dirs: List[str] = []  # temp dirs cleaned in close()
        ...
        elif self.config_model.pipeline.enable_document_pipeline:
            # Two-layer document pipeline (Docling + GLM-OCR)
            from glmocr.pipeline.document_pipeline import DocumentPipeline

            self._document_pipeline = DocumentPipeline(
                config=self.config_model.pipeline
            )
            self._document_pipeline.start()
            self.enable_layout = self.config_model.pipeline.enable_layout
            logger.info(
                "GLM-OCR initialized with two-layer DocumentPipeline "
                "(Docling text + GLM-OCR visuals)"
            )
        else:
            # Classic self-hosted mode: all-image pipeline
    ...
    def parse(
        ...
        **kwargs: Any,
        ...
        Supports image files (jpg, png, bmp, gif, webp), PDF, and office documents
        (.doc, .docx, .pptx, .xls, .xlsx, .ods). Office documents are converted to PDF via
        LibreOffice before processing; ensure LibreOffice is installed.
        ...
        resolved: List[str] = []
        use_doc_pipeline = self._document_pipeline is not None

        # Formats natively handled by DocumentPipeline (Docling supports these directly).
        # All other office formats need LibreOffice → PDF conversion first.
        _DOC_PIPELINE_NATIVE = frozenset({".pdf", ".docx", ".pptx", ".xlsx", ".png", ".jpg", ".jpeg", ".bmp", ".gif", ".webp", ".tiff", ".tif"})

        for idx, path in enumerate(images):
            p = path
            if p.startswith("file://"):
                p = p[7:]
            needs_conversion = os.path.isfile(p) and is_document_path(p)
            if use_doc_pipeline:
                # Only convert formats Docling can't handle natively
                needs_conversion = needs_conversion and Path(p).suffix.lower() not in _DOC_PIPELINE_NATIVE
            if needs_conversion:
                try:
                    pdf_path, temp_dir = convert_to_pdf(p)
                    resolved.append(pdf_path)
                    self._temp_dirs.append(temp_dir)
                except Exception as e:
                    logger.warning("Document conversion failed for %s: %s", path, e)
                    resolved.append(p)
            else:
                resolved.append(p)
        ...
        try:
            if self._use_maas:
                ocr_results = self._parse_maas(resolved, save_layout_visualization, **kwargs)
            elif use_doc_pipeline:
                ocr_results = self._parse_document_pipeline(resolved)
            else:
                ocr_results = self._parse_selfhosted(resolved, save_layout_visualization)

            return ocr_results[0] if _single else ocr_results
        finally:
            pass  # temp dirs cleaned in close() so result.save() can load originals
    ...
    def _parse_document_pipeline(
        self,
        sources: List[str],
    ) -> List[PipelineResult]:
        """Parse using the two-layer DocumentPipeline (Docling + GLM-OCR)."""
        results = []
        for source in sources:
            # Resolve file:// scheme
            p = source[7:] if source.startswith("file://") else source
            try:
                result = self._document_pipeline.process(p)
                results.append(result)
            except Exception as exc:
                logger.error(
                    "DocumentPipeline error for %s: %s", p, exc
                )
                results.append(
                    PipelineResult(
                        json_result=[],
